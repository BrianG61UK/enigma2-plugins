...
# Brian was here...
	def checkSimilarity(self, timer, name1, name2, shortdesc1, shortdesc2, extdesc1, extdesc2, force=False):
		...
		def titleMatch(p1, p2): # Tests two titles for a match
			...
			sp1 = ("".join(u for u in p1 if u not in ['\u0086', '\u0087'])).strip()	# Remove all section markers and strip leading and trailing spaces
			sp2 = ("".join(u for u in p2 if u not in ['\u0086', '\u0087'])).strip()	# Remove all section markers and strip leading and trailing spaces
		...
		def descMatch(p1, p2): # Tests two descriptions for a match
			...
			sp1 = "".join(u for u in p1 if u not in ['\u0086', '\u0087'])	# Remove all section markers
			sp2 = "".join(u for u in p2 if u not in ['\u0086', '\u0087'])	# Remove all section markers

			sp1 = stripLeadings(sp1)
			sp2 = stripLeadings(sp2)
